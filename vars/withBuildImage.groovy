...
def call(String profileName) {
    ...
    def kubernetesToken = kubernetesLogin(profileName)
...
def isCreateNamespace(profile) {
    return profile.deploy.create_namespace != null && profile.deploy.create_namespace.enabled != null && profile.deploy.create_namespace.enabled
}
